...
| `DisableMigrationDialog`  | `REG_DWORD`   | 0           | **Disable the stale-build migration dialog**: if set to 1, it will prevent your users from seeing the dialog that prompts users to stay on the latest version in the Visual Studio IDE. This policy works for Visual Studio 2026 and above. Insiders and Community scenarios aren't supported by this scenario |
