...
1. Download the patch specific to your version of Unreal Engine from the Unreal Engine source repository (`Engine/Extras/VisualStudioWorkspace`). To access the repository, see [accessing Unreal Engine source code on GitHub](https://www.unrealengine.com/ue-on-github). For example, `UnrealBuildTool-5.2.patch` patches Unreal Engine version 5.2.
